...
export interface LookupEntry {
	...
	relativePath: string;
	/** Geparste Liste, oder `null`, wenn die Datei kein gültiges CSV enthält. */
	lookup: LookupList | null;
	/** Referenzierbarer Name: der `# name:`-Metadaten-Wert, sonst der Dateiname ohne Endung. */
	name: string;
}

/** Liest und parst alle `.lkp`-Dateien im Workspace ein (inklusive ungesicherter Inhalte offener Editoren). */
	...
	return Promise.all(
		uris.map(async (uri): Promise<LookupEntry> => {
			...
			try {
				...
				return { uri, relativePath, lookup, name: lookup.name.trim() || fallbackName };
			} catch {
				return { uri, relativePath, lookup: null, name: fallbackName };
			}
		}),
	);
}
